...
<body>
	...
	<c:forEach begin="16" end="20" step="1" var="val2" varStatus="status">
		${val2} ${status.current} first: ${status.first} last:${status.last} ${status.count}<br>
	</c:forEach>
	<br>
	
	<%-- 서버에서 가져온 List<String> 출력 --%>	
	<c:forEach items="${fruits}" var="fruit" varStatus="status">
		${fruit} ::::: ${status.count} ${status.index} ${status.current} first: ${status.first} last:${status.last}<br>
	</c:forEach>
	<br>
